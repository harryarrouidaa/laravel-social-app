                ...
                @foreach ($comments as $comment)
                    <div class="w-full test-start p-10 flex justify-start items-center gap-5">
                        ...
                        <div class="flex justify-center items-center">
                            ...
                            <div class="text-start text-slate-600 pl-10">{{ $comment->content }}</div>
                        </div>
                    </div>
                @endforeach
                <form action="{{ route('post.comment.action', ['post_id' => $post_id]) }}" method="post" class="w-full flex justify-center my-20">
                    @csrf
                    <textarea name="content" class="textarea textarea-bordered w-[70%]"></textarea>
                    <button type="submit">Add</button>
                </form>
            </div>
        </div>
